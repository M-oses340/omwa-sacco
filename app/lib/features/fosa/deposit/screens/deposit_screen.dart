...
Future<bool?> showDepositSheet(BuildContext context, Map<String, dynamic> member) {
...
class _DepositSheetState extends State<_DepositSheet> {
  ...
  void _submit() {
    ...
    if (_useMpesa) {
      context.read<DepositBloc>().add(DepositInitiated(memberId: widget.member['id'], amount: amount));
    } else {
      context.read<DepositBloc>().add(CardDepositInitiated(memberId: widget.member['id'], amount: amount));
    }
  }

  Future<void> _openCheckout(DepositCheckoutReady state) async {
    final result = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => _CheckoutWebView(url: state.checkoutUrl)),
    );
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<DepositBloc, DepositState>(
      listener: (context, state) {
        if (state is DepositCheckoutReady) {
          _openCheckout(state);
            ...
            if (_successMessage != null) ...[
              ...
              Row(children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Deposit to FOSA', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                  Text('Choose payment method', style: TextStyle(fontSize: 12, color: cs.onSurface.withValues(alpha: 0.5))),
                ]),
              ]),
              const SizedBox(height: 20),
              Row(children: [
                Expanded(child: _MethodCard(label: 'M-Pesa', icon: Icons.phone_android, color: Colors.green, selected: _useMpesa, onTap: () => setState(() => _useMpesa = true))),
                const SizedBox(width: 12),
                Expanded(child: _MethodCard(label: 'Card', icon: Icons.credit_card, color: Colors.blue, selected: !_useMpesa, onTap: () => setState(() => _useMpesa = false))),
              ]),
              const SizedBox(height: 16),
              Wrap(
                spacing: 8,
                ...
                builder: (context, state) {
                  ...
                  return SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(
                        isLoading ? 'Processing...' : _useMpesa ? 'Pay via M-Pesa' : 'Pay via Card',
              ...
              if (!_useMpesa) ...[
                const SizedBox(height: 8),
                Text('You will be redirected to a secure payment page',
                    style: TextStyle(fontSize: 11, color: cs.onSurface.withValues(alpha: 0.5)), textAlign: TextAlign.center),
              ],
            ],
          ],
        ),
...
class _MethodCard extends StatelessWidget {
  ...
  const _MethodCard({required this.label, required this.icon, required this.color, required this.selected, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: selected ? color : Theme.of(context).colorScheme.outline.withValues(alpha: 0.3), width: selected ? 2 : 1),
          borderRadius: BorderRadius.circular(12),
        ),
        child: Column(children: [
          Icon(icon, color: selected ? color : Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.4), size: 24),
          const SizedBox(height: 4),
          Text(label, style: TextStyle(fontSize: 13, fontWeight: selected ? FontWeight.bold : FontWeight.normal, color: selected ? color : Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.6))),
        ]),
      ),
    );
  }
