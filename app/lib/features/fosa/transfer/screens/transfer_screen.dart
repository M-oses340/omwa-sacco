...
Future<bool?> showTransferSheet(BuildContext context, Map<String, dynamic> member) {
...
class _TransferSheetState extends State<_TransferSheet> with SingleTickerProviderStateMixin {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            TabBar(controller: _tabController, tabs: const [Tab(text: 'Internal (Sacco)'), Tab(text: 'External (Bank)')]),
            SizedBox(
              height: 380,
              child: TabBarView(
...
class _InternalTransferFormState extends State<_InternalTransferForm> {
  ...
  void _submit() {
    ...
    context.read<TransferBloc>().add(InternalTransferInitiated(
          ...
          child: Form(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              TextFormField(
                ...
                validator: (v) {
                  if (v == null || v.trim().isEmpty) return 'Enter member number';
                  if (v.trim().toUpperCase() == widget.member['member_number']) return 'Cannot transfer to yourself';
                  return null;
                },
              ),
              const SizedBox(height: 12),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(isLoading ? 'Processing...' : 'Transfer',
...
class _ExternalTransferFormState extends State<_ExternalTransferForm> {
  ...
  Future<void> _loadBanks() async {
    try {
      final res = await Supabase.instance.client.functions.invoke('bank-codes');
      final data = res.data as Map<String, dynamic>;
      final list = (data['banks'] as List).cast<Map<String, dynamic>>();
      setState(() {
        _banks = list.map((b) => {'code': b['code'].toString(), 'name': b['name'].toString()}).toList();
        _loadingBanks = false;
      });
    } catch (_) {
  ...
  void _submit() {
    if (!_formKey.currentState!.validate()) return;
    context.read<TransferBloc>().add(ExternalTransferInitiated(
      fromMemberId: widget.member['id'],
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _loadingBanks
                  ...
                  : DropdownButtonFormField<String>(
                      ...
                      items: _banks.map((b) => DropdownMenuItem(value: b['code'], child: Text(b['name']!, overflow: TextOverflow.ellipsis))).toList(),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(isLoading ? 'Processing...' : 'Transfer to Bank',
                      style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: cs.onPrimary)),
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.blue, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                ),
              ),
