...
export default function MemberDetail({ member, onClose }: Props) {
  ...
  const statement = txs.map((t, i) => {
    const isCredit = CREDIT_TYPES.has(t.transaction_type)
    const prev = i === 0 ? 0 : parseFloat(txs[i - 1].balance_after ?? txs[i - 1].balance_before ?? 0)
    return { ...t, isCredit, runningBalance: parseFloat(t.balance_after ?? prev) }
  }).reverse()
  ...
  function printStatement() {
    ...
    win.document.write(`
      <html><head><title>Statement - ${member.full_name}</title>
