...
export default function Settings() {
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-5">Manage member roles and access levels.</p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-6">
        <h3 className="font-medium text-gray-700 mb-3">Role Permissions</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs text-gray-600">
          {[
            ...
            <div key={r.role} className="bg-gray-50 rounded-lg p-3">
              <p className="font-semibold capitalize text-gray-800">{r.role}</p>
              <p className="text-gray-500 mt-0.5">{r.perms}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-medium text-gray-700">Member Roles</h3>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(m => (
                <tr key={m.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500 text-xs">{m.phone_number}</td>
                  ...
                  <td className="px-4 py-3">
                    <select
                      ...
                      className="border rounded px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-green-500 disabled:opacity-50"
                    >
