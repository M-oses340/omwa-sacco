...
Deno.serve(async (req: Request) => {
  // Create client inside handler to avoid cold-start session issues
  ...
  try {
    ...
    console.log('[CHECKOUT] user:', user?.id ?? 'null', 'err:', authError?.message ?? 'none')
    ...
    console.log('[CHECKOUT] member:', member?.id ?? 'null')
