Deno.serve(async (req: Request) => {
  function r(data: unknown, status = 200): Response {
  ...
  async function is(path: string, body: object) {
    const resp = await fetch(`${INTASEND_BASE}${path}`, {
      method: 'POST',
  ...
  try {
    ...
    if (body.action === 'deposit_card') {
      ...
      if (!ok || !data.url) {
        ...
        return r({ error: data?.errors?.[0]?.detail ?? data?.detail ?? data?.message ?? `IntaSend ${status}` }, 500)
      }
